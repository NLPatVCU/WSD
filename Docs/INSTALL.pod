##################################################################
#
#    INSTALLATION Instructions for the UMLS-SenseRelate Package
#    $Id: INSTALL.pod,v 1.2 2011/01/15 20:40:49 btmcinnes Exp $ 
##################################################################
...
UMLS-SenseRelate REQUIRES that the following software be installed. 
...
The exact location of where UMLS-SenseRelate will be installed 
...
To use the umls-senserelate-evaluation.pl program, you must install 
the scorer2 program from the SemEval 2010 All Words Disambiguation 
Task. To do this, there is an install.sh script in the External 
directory. This script takes a install location as input and installs
the scorer2 program in that location. For example:
