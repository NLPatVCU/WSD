...
# (Last Updated $Id: GlobalWord.pm,v 1.1 2014/10/29 14:36:40 btmcinnes Exp $)
#
...
sub assignSense {
    ...
    #  Add the possible cuis to the listing of possible cuis of the surrounding terms
	...
	foreach my $p (@{$f}) { 
	    ...
	    if(defined $trace) { print TRACE " Processing Group for Sense ($possible_sense)\n"; }
		...
		for my $j (0..$#array) { 
		    ...
		    if(defined $trace) { 
			print TRACE "    Relatedness($cui1, $cui2) = $score \n";
		    }
		}
	    }
	    ...
	    if(defined $trace) { 
		print TRACE " Overall similarity for $possible_sense = $avg_score\n"; 
	    }
    ...
    if(defined $trace) { 
	print TRACE "Returning Sense $max_sense with score $max_score\n";
	print TRACE "================================================\n";
    }
    return \%returnhash; 
}
